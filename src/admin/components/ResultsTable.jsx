...
export const ResultsTable = () => {
  ...
  const handleCleanFilter = () => {
    if (filterStatus.length === 0 && selectedApp === "" && searchedTerm === "")
      return;
  ...
  const handleDownloadPDF = async (aspirant) => {
    ...
    const docData = {
      aspirant: `${aspirant.first_name} ${aspirant.last_name_1} ${
    ...
    aRef.current.download = `${aspirant.first_name.replace(" ", "_")}_${
      aspirant.last_name_1
    }${
      aspirant.last_name_2 ? "_" + aspirant.last_name_2 : ""
    }_EXHA_Results.pdf`;
    aRef.current.click();
  };
  ...
  return (
    <>
      ...
      >
        <div ref={resultsMenuRef} className="hidden">
          Click Here!
        </div>
      </ResultsWithAnswersTable>
    </>
