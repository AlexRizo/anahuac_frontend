...
export const useExamStore = () => {
  ...
  const startLoadingExamLevel = async () => {
    try {
      const { data } = await anahuacApi.get(
        `/aspirants/${user.uid}/exam/level`
      );
  ...
  const startLoadingAllBlockQuestions = async (model) => {
    ...
    try {
      if (model === "lectura") {
        ...
        const block1 = data.questions
          .filter(
            (question) => question.relation === "672279a3a3f880dfc5b70a8a"
          )
          .sort(() => Math.random() - 0.5);
        const block2 = data.questions
          .filter(
            (question) => question.relation === "67228afb1d0fcdd16996d872"
          )
          .sort(() => Math.random() - 0.5);
        const block3 = data.questions
          .filter(
            (question) => question.relation === "6722937322a8c5bd203084a4"
          )
          .sort(() => Math.random() - 0.5);
        ...
        dispatch(
          onLoadQuestions(data.questions.sort(() => Math.random() - 0.5))
        );
        ...
        const blocks = [
          questions[questions.length - 3].questionNumber,
          questions[questions.length - 2].questionNumber,
          questions[questions.length - 1].questionNumber,
        ];

        const questionsBlock = questions.filter((question) =>
          blocks.includes(question.questionNumber)
        );
        const sortedQuestions = questions
          .filter((question) => !blocks.includes(question.questionNumber))
          .sort(() => Math.random() - 0.5);
  ...
  const startSaveLocalAnswer = (question) => {
    ...
    if (!Array.isArray(localExam)) {
      console.warn(
        "localExam no es un array. Reinicializando a un array vacío."
      );
  ...
  const startSavingExamAndNextLevel = async () => {
    ...
    if (totalResponded !== questions.length) {
      dispatch(setIsLoading("error"));
      return toast({
        title: "Contesta todas las preguntas",
        description: "No puedes continuar hasta que hayas respondido todas las preguntas.",
        variant: "destructive",
      });
    }

    try {
      const { data } = await anahuacApi.post(
        "/exam/saveandnext",
        { exam },
        { params: { exam_level } }
      );
